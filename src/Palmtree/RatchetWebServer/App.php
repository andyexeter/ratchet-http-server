<?php
namespace Palmtree\RatchetWebServer;

use Palmtree\RatchetWebServer\Http\Router;
...
class App
{
    ...
    public function __construct(
        ...
        $port = 8080,
        $address = '127.0.0.1',
        HttpServerInterface $notFoundController = null,
        LoopInterface $loop = null
    ) {
        if (extension_loaded('xdebug')) {
            trigger_error('XDebug extension detected. Remember to disable this if performance testing or going live!',
                E_USER_WARNING);
        }
        ...
        $router = new Router(new UrlMatcher($this->routes, new RequestContext), $notFoundController);
    ...
    public function route($path, ComponentInterface $controller, array $allowedOrigins = array(), $httpHost = null)
    {
        ...
        $this->routes->add('rr-' . ++$this->_routeCounter,
            new Route($path, array('_controller' => $decorated), array('Origin' => $this->httpHost), array(),
                $httpHost));
